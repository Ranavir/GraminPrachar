...
public class Utils {
	...
	public static final String TAG_SUCCESS_ACK = "1" ;
	public static final String TAG_FAILURE_ACK = "0" ;
	public static final String TAG_BLOCK_ACK = "-1" ;
	...
	public static String sendBulkDataToServer(JSONObject obj,String id) {
		...
		String status = null;
		try {		
			 System.out.println(TAG+"Bulk Data ::::::::::::::::: ");
	...
	public static boolean checkiInternet(Context context) {
		...
		NetworkInfo netInfo = conMgr.getActiveNetworkInfo();
		   if ( netInfo!= null && netInfo.isAvailable() &&   netInfo.isConnected()) {
		         ...
		         System.out.println(TAG+"Internet Connection Not Present");
		       return false;
		   }
	}
	...
	public static String getRegistrationAck(Context context,JSONObject jsonObjRegdDetails) {
		...
		if(checkiInternet(context)){
			try {
				...
				//System.out.println(TAG+"Bulk Data ::::::::::::::::: "+gprs_data.toString());
				ArrayList<NameValuePair> namevaluepair = new ArrayList<NameValuePair>();
				namevaluepair.add(new BasicNameValuePair("reqId","19"));
				...
				if(statusLine.getStatusCode() == HttpStatus.SC_OK){
					...
					String s = "" ;
					//line may be{SUCCESS/FAILURE/BLOCKED} CODES
					while ((s = br.readLine()) != null){
						line += s ;
					}
					System.out.println(TAG+"Utils----------line:"+line);
				}else{
					line = Utils.TAG_FAILURE_ACK ;
				}

			} catch (Exception e) {
				//msg="Unable to connect to the server.";
				e.printStackTrace();
				line = TAG_FAILURE_ACK ;
			}
		}else{
			AlertDialog.Builder alertDialog = new AlertDialog.Builder(context);
			alertDialog.setTitle("Message");
			alertDialog.setMessage("Internet connectivity unavailable!!!");
	...
	public static String getInputDataFromServer(String imei,Context context) {
		System.out.println(TAG+"Entry--->getInputDataFromServer");
		 if (android.os.Build.VERSION.SDK_INT > 9) {
			 StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
	         StrictMode.setThreadPolicy(policy);
		 }
		...
		if(checkiInternet(context)){
			try {
				...
				//System.out.println(TAG+"Bulk Data ::::::::::::::::: "+gprs_data.toString());
				 ...
				 System.out.println(TAG+"SERVER URL :: ------- " + SERVER_URL);
				 HttpPost httpPost = new HttpPost(SERVER_URL);
				 httpPost.setEntity(new UrlEncodedFormEntity(namevaluepair));
				 HttpResponse response = httpClient.execute(httpPost);
		         ...
		         if(statusLine.getStatusCode() == HttpStatus.SC_OK){
					 ...
					 String line = "";
					 while ((line= rd.readLine()) != null){
						 //System.out.println(TAG+"Bulk data sending Status : "+line);
						 status = line;
						 if(line.equals("FAILED")){
							...
							for(int i=0;i<mainArr.length();i++){
								...
								for (int j=0;j<songsArr.length();j++){
									...
									System.out.println(TAG+"row id : "+rowId);
									if(rowId>0){
										recordcount++;
									}else{
										recordFail++;
		...
		System.out.println(TAG+"Playlist Download Status :: "+msg);
		System.out.println(TAG+"EXIT--->getInputDataFromServer");
		return newStatus;		
	}
		...
		if(checkiInternet(context)){
			try {
		         ...
		         if(statusLine.getStatusCode() == HttpStatus.SC_OK){
					 ...
					 System.out.println(TAG+"111111111 :: "+songName);
					// OutputStream output = new FileOutputStream(MEDIA_PATH+songName);
					 ...
					 System.out.println(TAG+"9999999999 "+zis.available());
					 ZipEntry ze = zis.getNextEntry();
					 byte data[] = new byte[1024];
					 JSONArray arr = new JSONArray();
					 while(ze!=null){
						 System.out.println(TAG+"111111111111111");
				         ...
				         fos.close();   
				         ze = zis.getNextEntry();
				         System.out.println(TAG+"ze.......... "+ze);
					 }
					 //System.out.println(TAG+"");
					 zis.closeEntry();
				     zis.close();
					 System.out.println(TAG+"arr : "+arr.length());
					 if(arr.length()>0){
					 ...
					 System.out.println(TAG+"download complete......");
		         }else{
			...
			System.out.println(TAG+"no internet connection.................");
		}
	...
	public static String sendImagesToServer(Context context,String imei,String imgName,String cap_time){
		...
		if(checkiInternet(context)){
			try {
				 ...
				 System.out.println(TAG+"image path :: "+MEDIA_PATH);
	...
	public static File[] GetFiles() {
		...
		System.out.println(TAG+"=================================: external drive: "+ DirectoryPath);
	    ...
	    if(!f.exists()){
	    	System.out.println(TAG+"=================================: external drive not exist");
	    	f.mkdirs();   
	    }
	...
	public static String getFileNames(){
		System.out.println(TAG+"Get file names to be deleted............");
		File[] files = GetFiles();
	    JSONArray arr = new JSONArray();
	    try{	    		
		    ...
		    System.out.println(TAG+"List of mp3 files           : "+arr.toString());
		    System.out.println(TAG+"Total files in the directory: " + arr.length());
		...
		if(checkiInternet(androidBuildingMusicPlayerActivity)){
			try {
		         ...
		         if(statusLine.getStatusCode() == HttpStatus.SC_OK){
					 ...
					 String status="";
					 while ((line= rd.readLine()) != null){
						 System.out.println(TAG+"Media status :: "+line);
						 status=line;
					 }
			...
			try {
		         ...
		         if(statusLine.getStatusCode() == HttpStatus.SC_OK){
					 ...
					 while ((line= rd.readLine()) != null){
						 System.out.println(TAG+"status :: "+line);
						 if(line.equals("FAILED")){
							 System.out.println(TAG+"Failed to get dlt details..");
						 }else if(line.equals("NODATA")){
							 System.out.println(TAG+"No data for delete...");
						 }else{
	...
	public static void deleteFiles(JSONArray arrayFiles) {
	    	...
	    	for(int i=0;i<arrayFiles.length();i++){	    		
	    		try{
	    			...
	    			if(dltf.exists()){
				    	...
				    	System.out.println(TAG+"delete status : "+status);
	    			}			    	
	    ...
	    System.out.println(TAG+"dlt array size :: "+dltArr.length());
		if(dltArr.length()>0){
	...
	public static String getFlashNewsFromServer(String imei, Context context) {
		...
		System.out.println(TAG+"Flash News Download Status :: "+msg);
		return newStatus;		
	}
				         ...
				         /*try {
						    ...
						    player.setOnCompletionListener(new OnCompletionListener() {
					            ...
					            public void onCompletion(MediaPlayer mp) {
					            	System.out.println(TAG+"Flash new playing complete......................");
	 ...
	 * @date 29062016
	 **********************************************************/
	public static String getDistributorDetails(Context context, String strStateName) {
		String line = "";
		if (android.os.Build.VERSION.SDK_INT > 9) {
			StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
			StrictMode.setThreadPolicy(policy);
		...
		if(checkiInternet(context)){
			try {
				...
				ArrayList<NameValuePair> namevaluepair = new ArrayList<NameValuePair>();
				namevaluepair.add(new BasicNameValuePair("reqId","20"));
				namevaluepair.add(new BasicNameValuePair("state",strStateName));
	 ...
	 * @date 29062016
	 **********************************************************/
	public static String getVehicleDetails(Context context, String distributor_code) {
		String line = "";
		if (android.os.Build.VERSION.SDK_INT > 9) {
			StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
			StrictMode.setThreadPolicy(policy);
		...
		if(checkiInternet(context)){
			try {
				...
				ArrayList<NameValuePair> namevaluepair = new ArrayList<NameValuePair>();
				namevaluepair.add(new BasicNameValuePair("reqId","21"));
				namevaluepair.add(new BasicNameValuePair("distributor_code",distributor_code));
