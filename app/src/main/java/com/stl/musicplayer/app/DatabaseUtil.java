...
public class DatabaseUtil {
	...
	public long logRegdDetails(JSONObject input) {
		...
		try{
			...
			value.put("own_code",  		input.getString("own_code").trim());
	...
	public JSONObject getRegdDetails() {
		...
		try {
			...
			if (c.moveToLast()) {//move to the lates registration details from device
				obj = new JSONObject();
				...
				obj.put("own_code", c.getString(c.getColumnIndex("own_code")));
	...
	public ArrayList<HashMap<String, String>> checkSong(){
		...
		String path;
		if(isSDPresent){
			System.out.println(TAG+" isSDPresent::"+true);
		...
		System.out.println(TAG+"Media Path :: "+path);
		...
		try{
			...
			System.out.println(TAG+" songsList----------------"+songsList.toString());
		}catch(Exception e){
			e.printStackTrace();
		}
		return songsList;		
